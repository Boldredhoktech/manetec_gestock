...
import { ArrowLeft, Star, Package, ChevronRight } from 'lucide-react'
import { formatMontant } from '@/lib/utils'

export const metadata: Metadata = { title: 'Détail marque' }

interface Props { params: Promise<{ id: string }> }

export default async function PageDetailMarque({ params }: Props) {
    ...
    const { data: marque } = await adminClient
        .from('brands')
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-xl font-bold text-white">{marque.nom}</h1>
                        </div>
                        {marque.description && (
                            <p className="text-sm text-white/60 mt-0.5">{marque.description}</p>
                        )}
                    </div>
                ...
                <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                    ...
                    {!produits || produits.length === 0 ? (
                        <div className="text-center py-16 text-gray-400">
                            ...
                            <p className="text-sm">Aucun article pour cette marque.</p>
                        </div>
                    ) : (
                        <div className="divide-y divide-gray-50">
                            ...
                            Désactivé
                                        ...
                                        <div className="text-right shrink-0">
                                            <p className={`text-sm font-bold ${stock <= 0 ? 'text-red-600' : 'text-gray-700'}`}>
