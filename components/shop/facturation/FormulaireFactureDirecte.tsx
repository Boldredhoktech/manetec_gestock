...
export default function FormulaireFactureDirecte({ clients, produits, clientIdPreselectionne }: Props) {
    ...
    async function handleSoumettre() {
        setEnAttente(true)
        setErreur(undefined)
        const res = await creerFactureDirecte(
            clientId || null, objet, dateEcheance || null,
            remisePct, noteClient, lignes
        )
        setEnAttente(false)
        if (res?.erreur) { setErreur(res.erreur); return }
        setSucces(res.public_id!)
        setTimeout(() => router.push(`/admin/factures/${res.facture_id}`), 1500)
    }
        ...
        <div className="space-y-5">
            ...
            {succes && (
                <div className="flex items-center gap-3 bg-green-50 border border-green-200 text-green-700 rounded-lg px-4 py-3 text-sm">
                    ...
                    Facture {succes} créée avec succès. Redirection...
                </div>
            )}
            ...
            <div className="bg-card border border-border rounded-xl p-5 space-y-3">
                <h2 className="text-sm font-semibold text-foreground">Lignes de facture</h2>
